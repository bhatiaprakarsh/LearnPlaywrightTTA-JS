...
test.describe('Mettl login flow', () => {
  ...
  test('should show validation message for incorrect email or password', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /login to mettl/i }).click();

    const errorMessage = page.getByText(/You entered an incorrect email, password, or both/i);
  ...
  test('should login successfully with valid credentials', async ({ page }) => {
    ...
    await expect(page).not.toHaveURL(/\/corporate\/login$/i);
  });
});
